...
class MyCustomBaseTransformerLayer(BaseModule):
    ...
    def __init__(self,
        ...
        assert set(operation_order) & set(
            ['occ_cross_attn', 'self_attn', 'norm', 'ffn', 'cross_attn']) == \
            set(operation_order), f'The operation_order of' \
            f' {self.__class__.__name__} should ' \
            f'contains all four operation type ' \
            f"{['occ_cross_attn', 'self_attn', 'norm', 'ffn', 'cross_attn']}"
        ...
        **kwargs contains some specific arguments of attentions.
        ...
        for layer in self.operation_order:
            if layer == 'occ_cross_attn':
                temp_occ = temp_key = temp_value = query

                temp_bev = self.attentions[attn_index](
                    query,
                    temp_occ,
                    temp_occ,
                    ...
                    **kwargs)
                '''
                temp_bev = self.attentions[attn_index](  # bevocc
                    temp_occ,
                    query)
                '''
                attn_index += 1
