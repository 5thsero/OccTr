...
class Map(object):
    ...
    def kalman_update(self, boxes, scores, sem_scores):  # late
        '''
        :param boxes: xyzhwl_ry
        :return:
        '''
        boxes = boxes[:, [3, 4, 5, 0, 1, 2, 6]]   # hwlxyz ry
    ...
    def kalman_predict(self, ):  # early
        return torch.Tensor(self.kalman_filter.predict()).cuda()[:, [3, 4, 5, 0, 1, 2, 6]]

    def update_ids_scores_semantics(self, ids, scores, sem_scores):  # late
        '''

        :param ids:
        :param scores:
        更新了object_scores_map, object_semantics_map
        :return:
        '''
        # ----------------generate new id-----------------------------
    ...
    def update_boxes(self, boxes, ids):  # late
        '''

        :param ids:
        :param scores:
        更新了object_num_map，object_dof_map
        :return:
        '''
        # --------------update boxes------------------------
        ids = ids.long()
    ...
    def get_boxes(self, boxes, ids):  # early
...
class VoxelMapGrid(Map):     # TODO
    '''
    self.map
    '''

    def __init__(self, voxel_size, detect_thresh, voxel_area, score_decay, boxes_decay, sem_score_decay, num_classes,
                 world_coord):
        super(VoxelMapGrid, self).__init__(score_decay, boxes_decay, sem_score_decay, num_classes)
    ...
    def update_map(self, backbone_xyz, pose_camera_to_world=None):
        ...
        :param backbone_xyz: point_clouds_camera[0, :, :3]
        :param targets:
        ...
        discrete_pts = torch.round(world_pts / self.voxel_size.to(self.device)).type(torch.long)  # 2500，2
        discrete_pts[:, 0] = discrete_pts[:, 0] + self.voxel_area[0] / 2
        discrete_pts[:, 1] = discrete_pts[:, 1] + self.voxel_area[1] / 2
        valid_mask = ((discrete_pts[:, 0] >= 0) & (discrete_pts[:, 0] < self.voxel_area[0])) & \
                     ((discrete_pts[:, 1] >= 0) & (discrete_pts[:, 1] < self.voxel_area[1]))     # unobserved area，2500 bool
        valid_ind = torch.nonzero(valid_mask).squeeze(1)# (ind数） True索引
        # valid_mask_reshape = valid_mask.reshape(50, 50).int().cpu().numpy()
        # draw(valid_mask_reshape)
        value = self.voxel_map[discrete_pts[valid_ind, 0], discrete_pts[valid_ind, 1]]  # (long(valid_ind))
        #voxel_map = self.voxel_map.cpu().numpy()
        #draw(voxel_map)
        # voxel_map = torch.zeros(self.voxel_area)   value(ind数, N(2?), 3(xyz))
        self.voxel_map[discrete_pts[valid_ind, 0], discrete_pts[valid_ind, 1]] += 1
        # 对voxel_map中检索到的对应位置 +1
        mask_seen_ind = torch.nonzero(value > self.detect_thresh).squeeze(1)  # value大于detect_thresh = 3的ind 之前已经观测过的区域
        valid_mask.zero_()[valid_ind[mask_seen_ind]] = 1  # 除了mask_seen_ind对应为True,其他对应为False
