...
def boxes3d_to_bev_torch(boxes3d):
    """
    :param boxes3d: (N, 7) [x, y, z, h, w, l, ry]  中心点坐标xyz
    :return:                                            ↓
        boxes_bev: (N, 5) [x1, y1, x2, y2, ry]   两个对角点坐标x1y1, x2y2
    """
    boxes_bev = boxes3d.new(torch.Size((boxes3d.shape[0], 5)))

    cu, cv = boxes3d[:, 0], boxes3d[:, 2]   # bounding_box_3d.py : u-x , v-z
...
def _pc_bbox3_filter(bbox3, pc):
    ...
    v45 = bbox3[5] - bbox3[4]    # utils.bounding_box_3d  y z x
    v40 = bbox3[0] - bbox3[4]
    v47 = bbox3[7] - bbox3[4]
    point -= bbox3[4]
...
def _pc_bbox3_filter_batch(bbox3, pc):
    '''
    Find point cloud inside a bounding box.
    :param bbox3:  corner  8x3
    :param pc:
    :return:
    '''
    point = copy.deepcopy(pc).unsqueeze(0)
    v45 = bbox3[:, 5] - bbox3[:, 4]  # (-1,8,3)   # y轴
    v40 = bbox3[:, 0] - bbox3[:, 4]               # z
    v47 = bbox3[:, 7] - bbox3[:, 4]               # x
    point = point - bbox3[:, 4:5]  # 原点
    m0 = torch.matmul(point, v45.unsqueeze(-1))   # point的y,z,x投影
    m1 = torch.matmul(point, v40.unsqueeze(-1))
    m2 = torch.matmul(point, v47.unsqueeze(-1))
    cs = []
    for m, v in zip([m0, m1, m2], [v45, v40, v47]):   # point信息m0, m1, m2  边框信息v45, v40, v47
        c0 = 0 < m  # 投影点在坐标上大于0
        c1 = m < v.unsqueeze(1) @ v.unsqueeze(-1)  # 投影点小于边框长度
        c = c0 & c1
        cs.append(c)
    cs = cs[0] & cs[1] & cs[2]  # 都满足True, 其余点False
    cs = cs.squeeze(-1)
    return cs


def _pc_bbox3_filter_bev(bbox3, pc, coord='camera'):  # TODO
    '''
    ...
    point = 25*pc[:, [0, 1]].unsqueeze(0)  # x,y
    bbox3 = bbox3[:, :, [1, 0]]   # xz坐标
    v40 = bbox3[:, 0] - bbox3[:, 4]  # z轴 300,2
    v47 = bbox3[:, 7] - bbox3[:, 4]  # x轴
    point = point - bbox3[:, 4:5]    # 原点   对应300个bbox3 [300,2500,2]
    m0 = torch.matmul(point, v40.unsqueeze(-1))  # z,x投影
