...
class Box3DList(object):
    ...
    def _split_into_corners(self):
        ...
        elif (self.mode == "ry_lhwxyz" or self.mode == "xyzhwl_ry") and self.frame == "velodyne":  # 激光 右手定则
        ...
        elif (self.mode == "ry_lhwxyz" or self.mode == "xyzhwl_ry") and self.frame == "rect":   # 相机 左手定则
            if self.mode == "xyzhwl_ry":
                x, y, z, h, w, l, ry = self.bbox_3d.split(1, dim=-1)
            else:
                ry, l, h, w, x, y, z = self.bbox_3d.split(1, dim=-1)
    ...
    def cart2hom(self, pts_3d):
        ''' Input: nx3 points in Cartesian  笛卡尔   to
            Oupput: nx4 points in Homogeneous by pending 1  齐次化
        '''
    ...
    def transpose(self, method):  # 边界框上下翻转或者左右翻转
        """
    ...
    def rotate(self, angle, gt_alpha):
        bbox_3d = self.convert('xyzhwl_ry').bbox_3d  # 转换为xyzhwl_ry
